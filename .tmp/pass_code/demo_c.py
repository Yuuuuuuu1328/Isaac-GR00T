...
_DEFAULT_OUTPUT_JSONL = _REPO_ROOT / "deployment_scripts/ant/output/online_result_demo.jsonl"
...
def append_latency_record(output_path: str | Path, record: dict[str, Any]) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Aistudio HTTP client latency benchmark")
    ...
    parser.add_argument("--output-jsonl", type=str, default=str(_DEFAULT_OUTPUT_JSONL))
    return parser
...
def build_request_payload(
    ...
    *,
    ...
    return {"query": json.dumps(inner, ensure_ascii=True)}
...
def _print_run_latency(i: int, total_latency: float, infer_latency: float, network_latency: float) -> None:
    print(f"\n=== Run {i + 1} ===")
    ...
    ratio = (infer_latency / total_latency) * 100.0 if total_latency > 0.0 else 0.0
    print(f"Inference proportion: {ratio:.2f}%")


def _print_average_latency(
    *,
    total_latency_sum: float,
    infer_latency_sum: float,
    network_latency_sum: float,
    actual_runs: int,
) -> None:
    print("\n=== Average Latency ===")
    if actual_runs <= 0:
        print("Average total latency: 0.00000 ms")
        print("Average inference latency: 0.00000 ms")
        print("Average network latency: 0.00000 ms")
        print("Average inference proportion: 0.00%")
        return
    ...
    ratio = (infer_latency_sum / total_latency_sum) * 100.0 if total_latency_sum > 0.0 else 0.0
    print(f"Average inference proportion: {ratio:.2f}%")
...
def run_benchmark(args: argparse.Namespace) -> int:
    ...
    for run_index in range(args.measure_runs):
        ...
        error_message = _safe_ascii_text(body.get("errorMessage", ""))
        if result_code != 0:
            print(
        ...
        append_latency_record(
            args.output_jsonl,
            {
                ...
                "request_id": _safe_ascii_text(actual_request_id),
                "result_code": result_code,
                "error_message": error_message,
                "client_round_trip_ms": round(client_round_trip_ms, 4),
                "server_total_ms": round(server_total_ms, 4),
                "inference_latency_ms": round(pure_inference_ms, 4),
        ...
        _print_run_latency(run_index, client_round_trip_ms, pure_inference_ms, network_ms)

    _print_average_latency(
        total_latency_sum=total_latency_sum_ms,
        infer_latency_sum=infer_latency_sum_ms,
        network_latency_sum=network_latency_sum_ms,
    ...
    append_latency_record(
        args.output_jsonl,
        {
            ...
            "configured_measure_runs": args.measure_runs,
            "average_total_latency_ms": round(total_latency_sum_ms / actual_runs, 5) if actual_runs > 0 else 0.0,
            "average_inference_latency_ms": round(infer_latency_sum_ms / actual_runs, 5)
            if actual_runs > 0
            else 0.0,
            "average_network_latency_ms": round(network_latency_sum_ms / actual_runs, 5)
            if actual_runs > 0
            else 0.0,
            "average_inference_proportion_pct": round(
                (infer_latency_sum_ms / total_latency_sum_ms) * 100.0, 2
            )
