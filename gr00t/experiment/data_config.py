...
class NewInteractionGroup(BaseDataConfig):
    video_keys = ["video.ego_view"]
...
class NewInteractionGroupFuture(BaseDataConfig):
    video_keys = ["video.ego_view"]
    future_keys = ["video.future_view"]
    state_keys = ["state.single_arm"]
    action_keys = ["action.single_arm"]
    language_keys = ["annotation.task_index"]
    observation_indices = [0]
    ...
    def modality_config(self) -> dict[str, ModalityConfig]:
        ...
        future_modality = ModalityConfig(
            delta_indices=self.observation_indices,
            modality_keys=self.future_keys,
        )
        state_modality = ModalityConfig(
            delta_indices=self.observation_indices,
            modality_keys=self.state_keys,
...
DATA_CONFIG_MAP = {
    ...
    "new_interaction_group": NewInteractionGroup(),
    "new_interaction_group_future": NewInteractionGroupFuture(),
}
