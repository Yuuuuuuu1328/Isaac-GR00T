...
DEFAULT_SERVER_OUTPUT_JSONL = (
    _REPO_ROOT / "/home/jetson/Desktop/project/Isaac-GR00T/scripts/results/aistudio_server_e2e_result.jsonl"
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--server-output-jsonl",
        type=str,
        default=str(DEFAULT_SERVER_OUTPUT_JSONL),
        help="Server role: append one JSON line per request (same style as client --output-jsonl). Set to empty to disable.",
    )
    return parser
...
def send_request(
    *,
    ...
    # Server protocol: prefer {"status", "data"}; keep backward compatibility with {"status_code", "result"}.
    body = envelope.get("data", envelope.get("result", {}))
    ...
    response = SimpleNamespace(
        status_code=int(envelope.get("status", envelope.get("status_code", 200)) or 200),
...
def _server_request_meta_from_payload(payload: Any) -> dict[str, Any]:
    ...
    return {
        ...
        "framebuffer_size": int(parsed.get("framebuffer_size", 0) or 0),
    }


def parse_aistudio_query(payload: dict) -> dict:
    # 兼容两种请求格式：
    # 1) 原始客户端协议：{"query": "<json字符串>"}（双层 JSON）
    # 2) 已展开内层字段：{joint_angles: ..., framebuffer: ..., request_id: ...}
    if "query" in payload:
...
def create_app(
    *,
    ...
    server_output_jsonl: str = "",
    server_config_snapshot: dict[str, Any] | None = None,
):
    ws_path = ws_path if ws_path.startswith("/") else f"/{ws_path}"
    app = FastAPI(title="Aistudio WebSocket e2e Service")
    server_log_path = (server_output_jsonl or "").strip()
    config_snapshot = server_config_snapshot or {}
    ...
    async def ws_predict(websocket: WebSocket):
        ...
        while True:
            try:
                ...
                # 直接打印原始请求体（保持客户端发送的 JSON 字符串不做解析/重编码）
                print(raw_payload, flush=True)
                
                # 业务处理
                request_start_ns = time.perf_counter_ns()
                payload_for_meta: Any = None
                try:
                    payload_for_meta = json.loads(raw_payload)
                    if isinstance(payload_for_meta, dict) and len(payload_for_meta) <= 1:
                        print("Skip inference: payload has only one key", flush=True)
                        continue

                    parsed_for_check = parse_aistudio_query(payload_for_meta)
                    framebuffer = parsed_for_check.get("framebuffer", [])
                    if not isinstance(framebuffer, list) or len(framebuffer) == 0:
                        print("Skip inference: empty framebuffer", flush=True)
                        continue

                    response_body, timing = runtime.predict_with_timing(payload_for_meta)
                except Exception as exc:
                    try:
                        payload_for_meta = json.loads(raw_payload)
                    except Exception:
                        payload_for_meta = None
                ...
                inference_proportion_pct = (
                    round((pure_inference_ms / server_total_ms) * 100.0, 2)
                    if server_total_ms > 0.0
                    else 0.0
                )

                envelope = {
                    # Keep协议字段名：status + data
                    "status": 200,
                    "data": response_body,
                    "latency": {
                ...
                result_map_for_print = response_body.get("resultMap", {})
                if not isinstance(result_map_for_print, dict):
                    result_map_for_print = {}
                # 兼容：把 resultMap 的字段直接展开到根节点，
                # 以便其他客户端按扁平结构读取（例如直接读 action_sequence/predicted_coords_2d）。
                envelope.update(result_map_for_print)
                envelope.setdefault("predictedScore", 0.0)
                envelope.setdefault("picCount", 0)
                # 生成一个类似示例的 taskId（UTC 时间戳 + 3 位毫秒后缀）
                _now_utc = datetime.now(timezone.utc)
                _ms_suffix = str(int(_now_utc.timestamp() * 1000) % 1000).zfill(3)
                envelope["taskId"] = f"{_now_utc.strftime('%Y%m%d_%H%M%S')}_{_ms_suffix}"
                compact_response = {
                    "request_id": str(result_map_for_print.get("request_id", "")),
                    "resultCode": int(response_body.get("resultCode", 1)),
                    "errorMessage": _to_ascii_text(response_body.get("errorMessage", "")),
                    "latency": envelope["latency"],
                    "action_sequence": result_map_for_print.get("action_sequence", ""),
                }
                print(json.dumps(compact_response, ensure_ascii=False), flush=True)

                if server_log_path:
                    req_meta = _server_request_meta_from_payload(payload_for_meta)
                    if timing.get("request_id"):
                        req_meta.setdefault("request_id", str(timing["request_id"]))
                    if timing.get("device_id"):
                        req_meta.setdefault("device_id", str(timing["device_id"]))
                    result_map = response_body.get("resultMap")
                    if isinstance(result_map, dict):
                        if not req_meta.get("request_id"):
                            req_meta["request_id"] = str(result_map.get("request_id", ""))
                        if not req_meta.get("device_id"):
                            req_meta["device_id"] = str(result_map.get("device_id", ""))
                    result_code = int(response_body.get("resultCode", 1))
                    timestamp = datetime.now(timezone.utc).isoformat(timespec="seconds")
                    _append_jsonl_record(
                        server_log_path,
                        {
                            "record_type": "server_run",
                            "timestamp": timestamp,
                            "config": config_snapshot,
                            "request_meta": {
                                "phase": "server",
                                "request_id": req_meta.get("request_id", ""),
                                "device_id": req_meta.get("device_id", ""),
                                "framebuffer_size": req_meta.get("framebuffer_size", 0),
                                "ws_status_code": envelope.get("status", 200),
                            ...
                            "response": {
                                "resultCode": result_code,
                                "errorMessage": _to_ascii_text(response_body.get("errorMessage", "")),
                                "resultMap": response_body.get("resultMap", {}),
                            },
                        },
                    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.role == "server":
        ...
        server_log = (args.server_output_jsonl or "").strip()
        app = create_app(
            runtime=runtime,
            ws_path=args.ws_path,
            server_output_jsonl=server_log,
            server_config_snapshot=_json_safe(vars(args)) if server_log else None,
        )
