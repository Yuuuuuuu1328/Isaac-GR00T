...
echo "Building INT8 TensorRT engines for new_interaction_group (Jetson Orin NX)."
echo "Dynamic lengths, configurable batch size."

export PATH=/usr/src/tensorrt/bin:$PATH

# Default paths — override via environment variables
ONNX_ROOT=${ONNX_ROOT:-gr00t_onnx_int8}
ENGINE_ROOT=${ENGINE_ROOT:-gr00t_engine_int8}

# Precision — all INT8 for Orin NX (no FP8/nvFP4 support)
...
if [[ ! "$VIT_DTYPE" =~ ^(fp16|int8)$ ]]; then
    echo "Error: VIT_DTYPE must be 'fp16' or 'int8' (Orin NX), got '${VIT_DTYPE}'"
    exit 1
fi

if [[ ! "$LLM_DTYPE" =~ ^(fp16|int8)$ ]]; then
    echo "Error: LLM_DTYPE must be 'fp16' or 'int8' (Orin NX), got '${LLM_DTYPE}'"
    exit 1
fi

if [[ ! "$DIT_DTYPE" =~ ^(fp16|int8)$ ]]; then
    echo "Error: DIT_DTYPE must be 'fp16' or 'int8' (Orin NX), got '${DIT_DTYPE}'"
    exit 1
fi

# Check required ONNX files
required_onnx_files=(
...
trtexec --useCudaGraph --verbose --stronglyTyped --separateProfileRun --noDataTransfers \
    --onnx=${ONNX_ROOT}/action_head/vlln_vl_self_attention.onnx \
    --saveEngine=${ENGINE_ROOT}/vlln_vl_self_attention.engine \
    --minShapes=backbone_features:1x${MIN_LEN}x2048 \
    --optShapes=backbone_features:1x${OPT_LEN}x2048 \
    --maxShapes=backbone_features:${MAX_BATCH}x${MAX_LEN}x2048 \
    > ${ENGINE_ROOT}/vlln_vl_self_attention.log 2>&1

# DiT Model
echo "------------Building DiT Model (${DIT_DTYPE})--------------------"
trtexec --useCudaGraph --verbose --stronglyTyped --separateProfileRun --noDataTransfers \
    --onnx=${ONNX_ROOT}/action_head/DiT_${DIT_DTYPE}.onnx \
    --saveEngine=${ENGINE_ROOT}/DiT_${DIT_DTYPE}.engine \
    ...
    --maxShapes=sa_embs:${MAX_BATCH}x49x1536,vl_embs:${MAX_BATCH}x${MAX_LEN}x2048,timesteps_tensor:${MAX_BATCH} \
    > ${ENGINE_ROOT}/DiT_${DIT_DTYPE}.log 2>&1

# State Encoder
echo "------------Building State Encoder--------------------"
trtexec --useCudaGraph --verbose --stronglyTyped --separateProfileRun --noDataTransfers \
    --onnx=${ONNX_ROOT}/action_head/state_encoder.onnx \
    --saveEngine=${ENGINE_ROOT}/state_encoder.engine \
    --minShapes=state:1x1x64,embodiment_id:1 \
    --optShapes=state:1x1x64,embodiment_id:1 \
    --maxShapes=state:${MAX_BATCH}x1x64,embodiment_id:${MAX_BATCH} \
    > ${ENGINE_ROOT}/state_encoder.log 2>&1

# Action Encoder
echo "------------Building Action Encoder--------------------"
trtexec --useCudaGraph --verbose --stronglyTyped --separateProfileRun --noDataTransfers \
    --onnx=${ONNX_ROOT}/action_head/action_encoder.onnx \
    --saveEngine=${ENGINE_ROOT}/action_encoder.engine \
    --minShapes=actions:1x16x32,timesteps_tensor:1,embodiment_id:1 \
    --optShapes=actions:1x16x32,timesteps_tensor:1,embodiment_id:1 \
    --maxShapes=actions:${MAX_BATCH}x16x32,timesteps_tensor:${MAX_BATCH},embodiment_id:${MAX_BATCH} \
    > ${ENGINE_ROOT}/action_encoder.log 2>&1

# Action Decoder
echo "------------Building Action Decoder--------------------"
trtexec --useCudaGraph --verbose --stronglyTyped --separateProfileRun --noDataTransfers \
    --onnx=${ONNX_ROOT}/action_head/action_decoder.onnx \
    --saveEngine=${ENGINE_ROOT}/action_decoder.engine \
    --minShapes=model_output:1x49x1024,embodiment_id:1 \
    --optShapes=model_output:1x49x1024,embodiment_id:1 \
    --maxShapes=model_output:${MAX_BATCH}x49x1024,embodiment_id:${MAX_BATCH} \
    > ${ENGINE_ROOT}/action_decoder.log 2>&1

# VLM-ViT
echo "------------Building VLM-ViT (${VIT_DTYPE})--------------------"
trtexec --useCudaGraph --verbose --stronglyTyped --separateProfileRun --noDataTransfers \
    --onnx=${ONNX_ROOT}/eagle2/vit_${VIT_DTYPE}.onnx \
    --saveEngine=${ENGINE_ROOT}/vit_${VIT_DTYPE}.engine \
...
echo ""
echo "============================================================"
echo "INT8 TensorRT Engine Build Complete! (Orin NX)"
echo "============================================================"
