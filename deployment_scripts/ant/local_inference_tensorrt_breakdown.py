...
def _run_single_breakdown(policy, raw_obs: dict[str, Any], unsqueeze_dict_values) -> dict[str, float]:
    import contextlib
    import torch
...
def main() -> int:
    ...
    for _ in range(args.warmup_runs):
        _ = _run_single_breakdown(policy, copy.deepcopy(base_step_data), unsqueeze_dict_values)

    for run_index in range(args.measure_runs):
        metrics = _run_single_breakdown(policy, copy.deepcopy(base_step_data), unsqueeze_dict_values)
        records.append(
            LatencyRecord(
                meta={
                    "host": socket.gethostname(),
                    "script": "local_inference_tensorrt_breakdown.py",
                    "run_index": run_index,
