...
def _run_backbone_breakdown(backbone, backbone_inputs: BatchFeature) -> tuple[BatchFeature, dict[str, float]]:
    ...
    vit_embeds, vision_ms = measure_cuda_time_ms(
        lambda: backbone.eagle_model.extract_feature(eagle_input["pixel_values"])
    )
    ...
    def _run_llm():
        ...
        outputs = backbone.eagle_model.language_model(
...
def _run_action_head_breakdown(action_head, backbone_output: BatchFeature, action_input: BatchFeature) -> tuple[BatchFeature, dict[str, float]]:
    ...
    with torch.no_grad():
        processed_backbone_output, features_process_ms = measure_cuda_time_ms(
            lambda: action_head.process_backbone_output(backbone_output)
        )
        ...
        state_features, state_encoder_ms = measure_cuda_time_ms(
            lambda: action_head.state_encoder(action_input.state, embodiment_id)
        )
        ...
        for step_index in range(num_steps):
            ...
            action_features, action_encoder_ms = measure_cuda_time_ms(
                lambda tt=timesteps_tensor, current_actions=actions: action_head.action_encoder(
                    current_actions,
                    tt,
                    embodiment_id,
                )
            ...
            model_output, dit_ms = measure_cuda_time_ms(
                lambda states=sa_embs, tt=timesteps_tensor: action_head.model(
            ...
            pred, action_decoder_ms = measure_cuda_time_ms(
                lambda mo=model_output: action_head.action_decoder(mo, embodiment_id)
            )
        ...
        records.append(
            LatencyRecord(
                meta={
                    ...
                    "script": "local_inference_breakdown.py",
                    "run_index": run_index,
