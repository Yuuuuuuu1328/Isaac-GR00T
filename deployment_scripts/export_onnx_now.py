...
def _rewrite_layernorm_initializers_to_match_input(onnx_path, save_as_external_data=False):
    """
    For each LayerNormalization node, align scale/bias dtype to match the input dtype.
    INT8 quantization inserts Q/DQ nodes that promote some tensors from FP16 to FP32,
    but not all LayerNorm inputs are promoted equally:
      - layer_norm1 (before attention): input stays Half
      - layer_norm2 (after attention residual with DQ output): input becomes Float
    TensorRT requires input and scale/bias to have identical types.
    """
    ...
    try:
        ...
        print(f"[LayerNorm rewrite] Warning: shape inference failed: {e}")
    ...
    dtype_to_np = {TensorProto.FLOAT: np.float32, TensorProto.FLOAT16: np.float16}
    n_converted = 0
    ...
    for node in onnx_model.graph.node:
        ...
        input_dtype = type_map.get(node.input[0])
        if input_dtype not in dtype_to_np:
            continue

        target_np = dtype_to_np[input_dtype]
        for initializer_name in node.input[1:3]:
            initializer = initializer_map.get(initializer_name)
            if initializer is None or initializer.data_type == input_dtype:
                continue
            tensor = numpy_helper.to_array(initializer).astype(target_np)
            initializer.CopyFrom(numpy_helper.from_array(tensor, name=initializer_name))
            n_converted += 1

    print(f"[LayerNorm rewrite] Converted {n_converted} initializers to match input dtypes")
...
def export_eagle2_vit(
    ...
    if vit_dtype in ["fp8", "int8"]:
        ...
        for module in model.modules():
            if not isinstance(module, torch.nn.Linear) or not is_quantized_linear(module):
                continue
            module.input_quantizer._onnx_quantizer_type = "static"
            module.weight_quantizer._onnx_quantizer_type = "static"
    ...
    if vit_dtype == "int8":
        _rewrite_layernorm_initializers_to_match_input(vit_onnx_path, save_as_external_data=True)
    else:
        print(f"[ViT] Re-saving ONNX with external data storage: {vit_onnx_path}")
        vit_model = onnx.load(os.fspath(vit_onnx_path), load_external_data=True)
        onnx.save_model(
            vit_model,
            os.fspath(vit_onnx_path),
            save_as_external_data=True,
            all_tensors_to_one_file=False,
            size_threshold=0,
        )
